...
var baseUrlSelected = function (options) {
    download.download(options.modPath, {outputDir: options.downloadDirectory}, function (err, rewriteModPath) {
        ...
        rewrite.rewrite(rewriteModPath, options, function () {
...
var rewriteFinished = function (options) {
    ...
    console.log("When all should be available on the public internet (i.e. synced via dropbox), run this command:");
...
var writePreferences = function (options) {
    var preferencesToSave = _.pick(options, ['workshopFileInfosPath', 'rewriteBaseUrl', 'downloadDirectory']);
