...
var workshopFileInfoFound = function (err, result) {
    ...
    var installed_mods = mod_lister.list(null, {workshopFileInfosPath: path});
...
var baseUrlSelected = function (options) {
    download.download(options.modPath, {outputDir: options.downloadDirectory}, function (err, modPath) {
        ...
        rewrite.rewrite(modPath, options, rewriteFinished);
    });
};

var rewriteFinished = function (error, modPath) {
    console.log("Installing mod at %s to tabletop simulator", modPath);
};
